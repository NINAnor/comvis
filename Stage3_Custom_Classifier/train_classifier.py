...
import torch
from torch.utils import tensorboard  ### SEE PACKAGES DEPENDANCIES
...
def make_dataframe(
    ...
    # Get the list of images + their associated labels
    ...
    train_idx, val_idx = int(0.6 * len(df)), int(0.8 * len(df))

    # Assign 'train', 'val' or 'test' to 'train_val_test' column based on indices
    df.loc[:train_idx, 'train_val_test'] = 'train'
    df.loc[train_idx:val_idx, 'train_val_test'] = 'val'
    df.loc[val_idx:, 'train_val_test'] = 'test'
...
def create_dataloaders_from_labelstudio(
    ...
    for item in ["train", "val", "test"]:
        df_item = df[df["train_val_test"] == item]
